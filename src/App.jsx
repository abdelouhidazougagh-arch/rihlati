...
function TravelpayoutsWidget({ type }) {
  const containerRef = useRef(null);

  const configs = {
    flights: {
      src: `https://tp.media/content?currency=eur&trs=387480&shmarker=${MARKER}&locale=es&powered_by=true&border_radius=12&plain=false&color_button=%230057b8&color_focused=%230057b8&primary_override=%230057b8&promo_id=4132&campaign_id=100`,
    },
    hotels: {
      src: `https://tp.media/content?currency=eur&trs=387480&shmarker=${MARKER}&show_hotels=true&powered_by=true&locale=es&primary_override=%230057b8&color_button=%230057b8&color_icons=%230057b8&secondary=%23FFFFFF&dark=%23262626&light=%23FFFFFF&border_radius=12&plain=false&promo_id=7873&campaign_id=101`,
    },
    cars: {
      src: `https://tp.media/content?currency=eur&trs=387480&shmarker=${MARKER}&locale=es&powered_by=true&border_radius=12&plain=false&color_button=%230057b8&promo_id=3526&campaign_id=121`,
    },
  };
  ...
  useEffect(() => {
    ...
    script.src = configs[type]?.src || configs.flights.src;
    script.async = true;
  ...
  return <div ref={containerRef} className="tp-widget-container" />;
}
...
export default function App() {
  ...
  const [widgetType, setWidgetType] = useState("flights");
  ...
  const tabs = [
    { id: "vuelos", label: "Vuelos", icon: "✈️", widget: "flights" },
    { id: "hoteles", label: "Hoteles", icon: "🏨", widget: "hotels" },
    { id: "coches", label: "Coches", icon: "🚗", widget: "cars" },
    { id: "ferries", label: "Ferries", icon: "🚢", widget: "ferries" },
  ];

  const handleSearch = () => {
    const tab = tabs.find((t) => t.id === activeTab);
    setWidgetType(tab?.widget || "flights");
    setShowWidget(true);
    setTimeout(() => {
  ...
  return (
    <div className="app">
      ...
      <nav className="navbar">
        <div className="nav-inner">
          ...
          <div className="nav-links">
            ...
            <a href="#widget-section">Buscar</a>
          </div>
              ...
              {(activeTab === "vuelos" || activeTab === "ferries") && (
                <>
                  <AirportInput
                    ...
                    onChange={setOrigin}
                  />
                  <button className="swap-btn" onClick={() => { const tmp = origin; setOrigin(dest); setDest(tmp); }}>⇄</button>
                  <AirportInput
      ...
      {/* ── WIDGET SECTION ── */}
      {showWidget && (
        <section id="widget-section" className="widget-section">
          <div className="widget-inner">
            <h2 className="section-title">
              {tabs.find((t) => t.id === activeTab)?.icon} Resultados — {tabs.find((t) => t.id === activeTab)?.label}
            </h2>
            {activeTab === "ferries" ? (
              <div className="ferry-placeholder">
                <div className="ferry-icon">🚢</div>
                <h3>Búsqueda de ferries</h3>
                <p>Próximamente — Reserva tu ferry al mejor precio</p>
                <a
                  href={`https://c111.travelpayouts.com/click?shmarker=${MARKER}.ferries&promo_id=3791&source_type=customlink&type=click`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="ferry-cta"
                >
                  Ver opciones de ferry →
                </a>
              </div>
            ) : (
              <TravelpayoutsWidget type={widgetType} />
            )}
          </div>
        </section>
      )}
        ...
        <div className="section-inner">
          ...
          <div className="deals-grid">
            {DEALS.map((deal, i) => (
              <a
                key={i}
                href={`https://c111.travelpayouts.com/click?shmarker=${MARKER}.deal_${deal.from}_${deal.to}&promo_id=4132&source_type=customlink&type=click&custom_url=https://www.aviasales.es/search/${deal.from}${depDate.replace(/-/g, "") || ""}${deal.to}1`}
                target="_blank"
                rel="noopener noreferrer"
                className="deal-card"
              >
      ...
      <section id="destinos" className="destinations-section">
        <div className="section-inner">
          ...
          <div className="dest-grid">
            {[
              { name: "Marruecos", img: "🕌", desc: "Vuelos desde 35€", code: "MA" },
              { name: "Dubái", img: "🏙️", desc: "Vuelos desde 289€", code: "AE" },
              { name: "París", img: "🗼", desc: "Vuelos desde 29€", code: "FR" },
              { name: "Londres", img: "🎡", desc: "Vuelos desde 49€", code: "GB" },
              { name: "Estambul", img: "🕍", desc: "Vuelos desde 89€", code: "TR" },
              { name: "Argelia", img: "🌅", desc: "Vuelos desde 55€", code: "DZ" },
            ].map((d) => (
