...
function buildKiwiUrl(origin, dest, depDate, retDate, passengers, tripType) {
  ...
  // Wrap with Travelpayouts affiliate
  return `https://c111.travelpayouts.com/click?shmarker=${MARKER}&promo_id=4132&source_type=customlink&type=click&custom_url=${encodeURIComponent(kiwiUrl)}`;
}
...
export default function App() {
  ...
  const handleSearch = () => {
    ...
    if (activeTab === "hoteles") {
      ...
      const hotelUrl = `https://c111.travelpayouts.com/click?shmarker=${MARKER}&promo_id=7873&source_type=customlink&type=click&custom_url=${encodeURIComponent(`https://www.booking.com/searchresults.es.html?ss=${dest.city}&checkin=${depDate}&checkout=${retDate}&group_adults=${passengers}`)}`;
    ...
    if (activeTab === "coches") {
      ...
      const carUrl = `https://c111.travelpayouts.com/click?shmarker=${MARKER}&promo_id=3526&source_type=customlink&type=click`;
      window.open(carUrl, "_blank");
    }

    if (activeTab === "ferries") {
      window.open(`https://c111.travelpayouts.com/click?shmarker=${MARKER}&promo_id=3791&source_type=customlink&type=click`, "_blank");
    }
  };
